...
function sparqlEngine(queryParams) {
  ...
  if (!inputStock) return { results: [], query: "", explanation: "" };
  ...
  // Execute the query logic in JS
  const candidates = ALL_SYMBOLS
    .filter(sym => sym !== inputSymbol)
    .map(sym => EGX_DATA[sym])
    .filter(s => {
      if (!s.sector) return false;
      if (shariahOnly && !s.shariah) return false;
      if (s.vol20d !== undefined && s.vol20d > maxVol) return false;
      return true;
    })
    .map(s => {
      const sameSector = s.sector === inputStock.sector ? 0.4 : 0.0;
      const isPeer = (inputStock.peers || []).includes(s.symbol) ? 0.5 : 0.0;
      const momentumScore = (s.momentum !== undefined ? s.momentum * momentumBoost : 0.0);
      const valuationBonus = (s.pb !== undefined && s.pb > 0 && s.pb < 3) ? 0.3 : 0.0;
      const lowVolBonus = (s.vol20d !== undefined && s.vol20d < maxVol * 0.5) ? 0.2 : 0.0;
      ...
      return { ...s, score, reasons, sameSector: sameSector > 0 };
    })
  ...
  return { results: candidates, query, explanation };
}
...
export default function EGXRecommender() {
  ...
  const { results, query, explanation } = useMemo(() => {
    if (!inputSymbol) return { results: [], query: "", explanation: "" };
  ...
  return (
    <div style={{padding:"1.5rem 0",fontFamily:"var(--font-sans)"}}>
      ...
      <p style={{fontSize:13,color:"var(--color-text-secondary)",margin:"0 0 1.5rem"}}>SPARQL-based · EGX ontology · 197 listed stocks</p>
      ...
      {ran && inputStock && (
        <>
          <div style={{background:"var(--color-background-secondary)",borderRadius:"var(--border-radius-md)",padding:"10px 14px",marginBottom:"1rem",fontSize:12,color:"var(--color-text-secondary)"}}>
            <i className="ti ti-info-circle" style={{marginRight:6,verticalAlign:-2,fontSize:14}} aria-hidden="true"/>
            {explanation}
          </div>
          ...
          <div style={{marginTop:"1.5rem"}}>
            <button onClick={() => setShowQuery(!showQuery)} style={{fontSize:12,display:"flex",alignItems:"center",gap:6}}>
              <i className={`ti ti-code`} style={{fontSize:14}} aria-hidden="true"/>
      ...
      {!ran && (
        <div style={{textAlign:"center",padding:"2.5rem 1rem",color:"var(--color-text-tertiary)",fontSize:14,border:"0.5px dashed var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)"}}>
          <i className="ti ti-search" style={{fontSize:28,display:"block",marginBottom:8}} aria-hidden="true"/>
